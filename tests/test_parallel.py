...
# The mosaic-fitting entry points live in ``linum_basic.fit``, which is added
# by a later pull request in the chain. Until that lands, the four fit-driven
# tests below are skipped so this bundle stays self-contained and CI-green.
...
try:  # pragma: no cover - exercised once fit lands in the stacked PR
    from linum_basic.fit import MosaicFit, apply_fit, fit_mosaic

    _FIT_AVAILABLE = True
except ImportError:  # pragma: no cover
    MosaicFit: Any = lambda *a, **k: None  # noqa: E731
    apply_fit: Any = lambda *a, **k: None  # noqa: E731
    fit_mosaic: Any = lambda *a, **k: None  # noqa: E731
    _FIT_AVAILABLE = False

requires_fit = pytest.mark.skipif(
    not _FIT_AVAILABLE,
    reason="linum_basic.fit is added by a later pull request in the chain",
)
