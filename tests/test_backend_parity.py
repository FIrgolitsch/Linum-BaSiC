...
# The low-level ALM solver lives in ``linum_basic._alm``, which is added by a
# later pull request in the chain. Until that lands, the ALM-parity test class
# below is skipped so this bundle stays self-contained and CI-green.
try:  # pragma: no cover - exercised once _alm lands in the stacked PR
    from linum_basic._alm import inexact_alm_l1 as _inexact_alm_l1  # noqa: F401

    _ALM_AVAILABLE = True
except ImportError:  # pragma: no cover
    _ALM_AVAILABLE = False

requires_alm = pytest.mark.skipif(
    not _ALM_AVAILABLE,
    reason="linum_basic._alm is added by a later pull request in the chain",
)
