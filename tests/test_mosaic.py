...
# ``MosaicGrid.from_ome_zarr`` reads OME-Zarr stores through
# ``linum_basic.io.zarr``, which is added by a later pull request in the chain.
# Until that lands, the from_ome_zarr test classes below are skipped so this
# bundle stays self-contained and CI-green.
try:  # pragma: no cover - exercised once io.zarr lands in the stacked PR
    from linum_basic.io.zarr import load_ome_zarr as _load_ome_zarr  # noqa: F401

    _IO_ZARR_AVAILABLE = True
except ImportError:  # pragma: no cover
    _IO_ZARR_AVAILABLE = False

requires_io_zarr = pytest.mark.skipif(
    not _IO_ZARR_AVAILABLE,
    reason="linum_basic.io.zarr is added by a later pull request in the chain",
)
