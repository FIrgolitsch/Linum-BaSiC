...
# These two operator docs are added by a later pull request in the chain. Until
# they land, every test in this module would fail at fixture-read time, so the
# whole module is skipped. The guard becomes a no-op once the docs ship.
pytestmark = pytest.mark.skipif(
    not (TUNING_DOC.exists() and RUNBOOKS_DOC.exists()),
    reason="docs/tuning.md and docs/runbooks.md are added by a later pull request in the chain",
)
